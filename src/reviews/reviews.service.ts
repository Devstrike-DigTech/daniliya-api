...
export class ReviewsService {
  ...
  async create(userId: string, dto: CreateReviewDto) {
    ...
    const purchased = await this.prisma.order.findFirst({
      where: {
        customerId: userId,
        status: { in: [OrderStatus.CONFIRMED, OrderStatus.DELIVERED, OrderStatus.COMPLETED] },
  ...
  async list(status?: ReviewStatus) {
    const rows = await this.prisma.review.findMany({
      ...
      include: { author: { select: { firstName: true } }, product: { select: { title: true } } },
      orderBy: { createdAt: 'desc' },
    });
    return rows.map((r) => ({ ...this.present(r), reviewer: r.author.firstName, product: r.product?.title }));
  }

  flag(adminId: string, reviewId: string, dto: FlagReviewDto, ip?: string) {
    return this.moderate(adminId, reviewId, ReviewStatus.FLAGGED, ip, dto.reason);
  }
  ...
  private async moderate(
    ...
    const updated = await this.prisma.review.update({
      where: { id: reviewId },
      data: { status: to, flagReason: to === ReviewStatus.FLAGGED ? reason : review.flagReason },
    });
